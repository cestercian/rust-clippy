...
// Regression test: non-const chunk sizes must not trigger the lint in for-loop / iterator-method
// use sites either. The new applicability handling only kicks in once the lint has already
// decided to fire (i.e. only for const-evaluable arguments), so it must have no effect here.
fn issue17515_negative(slice: &[u8], n: usize) {
    for _ in slice.chunks_exact(n) {}
    slice.chunks_exact(n).for_each(|_| {});
}
